...
function renderGenerate(){
  ...
  return [
    ...
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\u2728 根据关键词生成文章</h3>',
    '<div style="display:grid;gap:14px;max-width:600px">',
    '<div><label style="display:block;font-size:12px;color:var(--mt);margin-bottom:6px">文章主题 / 关键词 *</label>',
    '<input id="gen-kw" type="text" class="fi" placeholder="例如: DeepSeek R2发布、AI芯片竞争、房价走势..."></div>',
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">',
    '<div><label style="display:block;font-size:12px;color:var(--mt);margin-bottom:6px">分类</label>',
    '<select id="gen-cat" class="fi">'+catOptions+'</select></div>',
    '<div><label style="display:block;font-size:12px;color:var(--mt);margin-bottom:6px">标签（逗号分隔）</label>',
    '<input id="gen-tags" type="text" class="fi" placeholder="例如: AI, 大模型, 科技"></div>',
    '</div>',
    '<div><button class="btn bg2" onclick="doGenerate()">\u26a1 生成文章</button>',
    '<span style="font-size:12px;color:var(--dt);margin-left:12px">生成后可进入「文章管理」编辑完整内容</span></div>',
    '</div></div>',
    '<div class="card" style="padding:24px">',
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\ud83d\udce6 批量生成</h3>',
    '<div style="display:grid;gap:14px;max-width:600px">',
    '<div><label style="display:block;font-size:12px;color:var(--mt);margin-bottom:6px">输入多个关键词（每行一个）</label>',
    '<textarea id="batch-kw" class="fi" placeholder="DeepSeek R2发布\n新能源汽车补贴政策\n美联储利率决议\n华为Mate70系列" style="min-height:120px"></textarea></div>',
    '<div><label style="display:block;font-size:12px;color:var(--mt);margin-bottom:6px">统一分类</label>',
    '<select id="batch-cat" class="fi" style="max-width:200px">'+catOptions+'</select></div>',
    '<div><button class="btn bp" onclick="doBatchGenerate()">\u26a1 批量生成</button></div>',
    '</div></div>',
    '<div class="card" style="padding:24px">',
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\ud83d\udd25 快速生成</h3>',
    '<div style="display:flex;flex-wrap:wrap;gap:8px">'+hotBtns+'</div>',
    '</div>',
    '</div>'
  ].join('');
}
...
function renderConfig(){
  var c=state.siteConfig;
  function inp(id,label,placeholder,value,maxw){
    return'<div><label style="display:block;font-size:12px;color:var(--mt);margin-bottom:6px">'+label+'</label><input id="cfg-'+id+'" type="text" value="'+esc(value)+'" class="fi" placeholder="'+(placeholder||'')+'" style="max-width:'+(maxw||'560px')+'"></div>';
  }
  function area(id,label,placeholder,value){
    return'<div><label style="display:block;font-size:12px;color:var(--mt);margin-bottom:6px">'+label+'</label><textarea id="cfg-'+id+'" class="fi" placeholder="'+(placeholder||'')+'" style="max-width:560px">'+esc(value)+'</textarea></div>';
  }
  return [
    '<div class="fa" style="display:grid;gap:20px">',
    '<div class="card" style="padding:24px">',
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\ud83d\udd8a 网站基本信息</h3>',
    '<div style="display:grid;gap:14px;max-width:560px">',
    inp('siteName','网站名称'),
    area('siteDesc','网站描述','一句话描述你的网站'),
    inp('footerText','底部版权文字'),
    '</div></div>',
    '<div class="card" style="padding:24px">',
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\ud83d\udcc4 关于我们页面</h3>',
    '<div style="display:grid;gap:14px;max-width:560px">',
    inp('aboutTitle','页面标题'),
    area('aboutContent','关于我们内容','输入关于我们页面的内容...'),
    '</div></div>',
    '<div class="card" style="padding:24px">',
    '<button class="btn bg2" onclick="saveSiteConfig()">\ud83d\udcbe 保存所有配置</button>',
    '<span style="font-size:12px;color:var(--dt);margin-left:12px">保存后需同步到云端才能生效</span>',
    '</div>',
    '</div>'
  ].join('');
}
...
function renderSources(){
  var srcBtns=Object.keys(SRC_MAP).map(function(k){
    var enabled=state.sources[k];
    return'<button class="btn '+(enabled?'bp':'bg3')+'" style="cursor:pointer;font-size:13px" onclick="state.sources[\u0027'+k+'\u0027]=!state.sources[\u0027'+k+'\u0027];renderSources()">'+SRC_MAP[k]+' '+(enabled?'\u2713':'\u25cb')+'</button>';
  }).join('');
  return [
    '<div class="fa" style="display:grid;gap:20px">',
    '<div class="card" style="padding:24px">',
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\ud83d\udd25 数据源开关</h3>',
    '<p style="font-size:13px;color:var(--mt);margin:0 0 16px">开启/关闭数据源采集，关闭的数据源将在自动生成时跳过。</p>',
    '<div style="display:flex;flex-wrap:wrap;gap:10px">'+srcBtns+'</div>',
    '</div>',
    '<div class="card" style="padding:24px">',
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\u23f0 定时自动生成</h3>',
    '<div style="display:flex;align-items:center;gap:16px;margin-bottom:16px">',
    '<label style="display:flex;align-items:center;gap:8px;cursor:pointer;font-size:14px">',
    '<input type="checkbox" id="auto-run" '+(state.autoRun?'checked':'')+' onchange="state.autoRun=this.checked">',
    '<span>启用每日自动生成</span>',
    '</label>',
    '</div>',
    '<div style="display:grid;gap:12px;max-width:300px">',
    '<div><label style="display:block;font-size:12px;color:var(--mt);margin-bottom:6px">每日运行时间</label>',
    '<input type="time" id="run-time" value="'+state.runTime+'" class="fi" onchange="state.runTime=this.value"></div>',
    '<div><label style="display:block;font-size:12px;color:var(--mt);margin-bottom:6px">每次生成文章数</label>',
    '<input type="number" value="'+state.perRun+'" class="fi" min="1" max="30" id="per-run" onchange="state.perRun=parseInt(this.value)||10"></div>',
    '</div>',
    '<div style="margin-top:16px;padding:14px;background:rgba(99,102,241,.1);border-radius:10px;font-size:13px;color:var(--mt);line-height:1.7">',
    '\ud83d\udca1 Mac 定时任务已配置在系统层（launchd），每次生成后会同步到 GitHub，Vercel 自动部署更新。',
    '</div>',
    '</div>',
    '</div>'
  ].join('');
...
function renderContact(){
  var c=state.siteConfig;
  function inp(id,label,placeholder){
    return'<div><label style="display:block;font-size:12px;color:var(--mt);margin-bottom:6px">'+label+'</label><input id="cfg-'+id+'" type="text" value="'+esc(c[id]||'')+'" class="fi" placeholder="'+(placeholder||'')+'"></div>';
  }
  return [
    '<div class="fa" style="display:grid;gap:20px">',
    '<div class="card" style="padding:24px">',
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\ud83d\udcde 联系方式设置</h3>',
    '<p style="font-size:13px;color:var(--mt);margin:0 0 16px">设置显示在「联系方式」页面的信息。</p>',
    '<div style="display:grid;gap:14px;max-width:560px">',
    inp('contactTitle','页面标题'),
    inp('contactEmail','\ud83d\udce7 邮箱地址','example@example.com'),
    inp('contactWechat','\ud83d\udcac 微信','微信号（选填）'),
    inp('contactQQ','\ud83d\udcf1 QQ号码','QQ号（选填）'),
    '</div></div>',
    '<div class="card" style="padding:24px">',
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\ud83c\udf0e 社交媒体</h3>',
    '<div style="display:grid;gap:14px;max-width:560px">',
    inp('socialWeibo','\ud83d\udda2 微博主页','https://weibo.com/你的用户名'),
    inp('socialZhihu','\ud83d\udca1 知乎主页','https://www.zhihu.com/people/你的用户名'),
    '</div></div>',
    '<div class="card" style="padding:24px">',
    '<button class="btn bg2" onclick="saveSiteConfig()">\ud83d\udcbe 保存联系方式</button>',
    '</div>',
    '</div>'
  ].join('');
}
...
function renderDeploy(){
  return [
    '<div class="fa" style="display:grid;gap:20px">',
    '<div class="card" style="padding:24px">',
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\ud83c\udf10 当前部署状态</h3>',
    '<div style="display:grid;gap:12px;margin-bottom:16px">',
    '<div style="display:flex;align-items:center;gap:12px;padding:14px;background:var(--c2);border-radius:10px">',
    '<span style="font-size:24px">\u2705</span>',
    '<div><p style="margin:0;font-weight:600">网站已成功部署</p><p style="margin:0;font-size:13px;color:var(--mt)">Vercel 自动 HTTPS \u00b7 全球 CDN \u00b7 免费域名</p></div>',
    '</div>',
    '<div style="display:flex;gap:10px;flex-wrap:wrap">',
    '<a href="'+SITE+'" target="_blank" class="btn bp" style="text-decoration:none">\ud83c\udf10 访问网站</a>',
    '<a href="https://vercel.com/dashboard" target="_blank" class="btn bg3" style="text-decoration:none">\ud83d\udcca Vercel 后台</a>',
    '</div>',
    '</div>',
    '</div>',
    '<div class="card" style="padding:24px">',
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\u2601\ufe0f 同步到云端</h3>',
    '<p style="font-size:13px;color:var(--mt);margin:0 0 16px;line-height:1.6">点击下方按钮，将本地修改同步到 GitHub，Vercel 检测到更新后会自动部署（1-2分钟生效）。</p>',
    '<div style="display:flex;gap:10px;margin-bottom:16px">',
    '<button class="btn bg2" onclick="syncNow()" '+(state.syncing?'disabled':'')+'>',
    state.syncing?'\u23f3 同步中...':'\u2601\ufe0f 立即同步',
    '</button>',
    '<button class="btn bg3" onclick="loadData()">\ud83d\udd04 刷新数据</button>',
    '</div>',
    '<div style="padding:14px;background:rgba(16,185,129,.1);border-radius:10px;border:1px solid rgba(16,185,129,.3)">',
    '<p style="margin:0 0 6px;font-weight:600;color:var(--g);font-size:13px">\ud83d\udca1 同步流程</p>',
    '<p style="margin:0;font-size:12px;color:var(--mt);line-height:1.7">1. 提交代码到 GitHub \u2192 2. Vercel 检测到更新 \u2192 3. 自动构建部署 \u2192 4. 1-2 分钟后生效</p>',
    '</div>',
    '</div>',
    '<div class="card" style="padding:24px">',
    '<h3 style="font-size:16px;font-weight:600;margin:0 0 16px">\ud83d\udd27 部署信息</h3>',
    '<div style="display:grid;gap:10px;font-size:13px">',
    '<div style="display:flex;gap:8px"><span style="color:var(--mt);min-width:80px">GitHub:</span><code style="color:var(--a)">github.com/mubaiqaq666-png/seo-content-site</code></div>',
    '<div style="display:flex;gap:8px"><span style="color:var(--mt);min-width:80px">分支:</span><code>main</code></div>',
    '<div style="display:flex;gap:8px"><span style="color:var(--mt);min-width:80px">平台:</span><code>Vercel (自动 HTTPS)</code></div>',
    '<div style="display:flex;gap:8px"><span style="color:var(--mt);min-width:80px">域名:</span><code>seo-content-site.vercel.app</code></div>',
    '</div>',
    '</div>',
    '</div>'
  ].join('');
